...
import { 
  ...
  Droplets, Wand2, Loader2
} from 'lucide-react';
import DownloadModal from './DownloadModal';
import ImageCropper from './ImageCropper';
import { removeBackground } from '../utils/imageProcessor';
import { digitizeHandwriting } from '../services/aiService';
...
const PreviewEditor: React.FC<PreviewEditorProps> = ({ images, onBack, onUpdateImage, onRemoveImage, onMoveImage }) => {
  ...
  const ToolbarBtn = ({ icon: Icon, label, active = false, onClick, disabled, loading }: any) => (
    <button 
      onClick={onClick}
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-2 flex items-center justify-between shadow-sm z-20 overflow-x-auto">
        ...
        <div className="flex items-center gap-1 px-2">
           <div className="flex items-center gap-1 mr-4 border-r border-slate-100 pr-2">
             <ToolbarBtn icon={Undo} label="Undo" disabled />
             <ToolbarBtn icon={Redo} label="Redo" disabled />
           </div>
           
           <ToolbarBtn icon={MousePointer2} label="Select" active />
           <ToolbarBtn icon={Crop} label="Crop" onClick={() => setEditingImageId(currentImage.id)} />
           
           {/* Magic Tools */}
           <div className="mx-2 flex items-center gap-1 bg-blue-50/50 rounded-lg px-1 border border-blue-100">
             <ToolbarBtn 
                icon={Droplets} 
                label="Remove BG" 
                 ...
                 {(isProcessingBg || isDigitizing) && (
                   <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-20 flex flex-col items-center justify-center gap-3">
                     ...
                     <span className="text-sm font-semibold text-blue-700 animate-pulse">
                        {isDigitizing ? 'Digitizing Handwriting with AI...' : 'Removing Background...'}
                     </span>
                   </div>
                 )}
